...
class apitransacciones{
    ...
    function obtenertransacciones($id_usuario){
        $transaccion = new transacciones();
        $transaccions = array();
        $transaccions["items"] = array();

        $res = $transaccion->obtenertransacciones($id_usuario);
        ...
        if($res->rowCount()){
            ...
            while ($row = $res->fetch(PDO::FETCH_ASSOC)){
                ...
                array_push($transaccions["items"], $item);
                $cont = $cont + 1;
            }
        
            $this->printJSON($transaccions);
        }else{
